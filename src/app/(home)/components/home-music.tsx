...
const HomeMusic = ({ userId }: { userId: string }) => {
  const { data, isPending, isError } = useGetPostTodayEmotionByIdQuery(userId);

  if (isPending || data === undefined || isError || data.length == 0) {
    return (
  ...
  return (
    <div className="my-4 flex flex-col gap-4">
      ...
      <div className="flex flex-row items-center gap-4 rounded-xl border bg-primary-50 p-4">
        <EmotionImage src={checkEmotion(data[0]?.post_emotion)} size="xs" />
        <div className="flex flex-1 flex-col justify-between">
          <a
            href={`${EMOTIONSMUSIC[data[0]?.post_emotion]}`}
            target="_blank"
            rel="noopener noreferrer"
            className="block rounded-xl bg-primary-50 p-4"
          >
            <div className="font-bold">
              {EMOTIONS_QUERY[data[0]?.post_emotion]}날 추천하는 노래
            </div>
          </a>
        </div>
      </div>
